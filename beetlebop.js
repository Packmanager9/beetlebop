...
//         console.log(gamepadAPI.axesStatus[1]*gamepadAPI.axesStatus[0]) //debugging
    ...
    class Head{
        constructor(color){
            ...
            this.angle = 0
            this.size = 24
            this.step = Math.PI*2/this.size
            for(let t = 0;t<Math.ceil(this.size*.5);t++){
                this.distances.push(this.min +(Math.random()*this.max))
            }
            this.rebuild()
            this.click = 0
        }
        rebuild(){
            this.click++
            if(this.click%10 == 0 || this.click%10 == 1 || this.click%10 == 2){
                this.distances.push(this.distances[0])
                this.distances.splice(0,1)
            }
            this.points = []
            for(let t = 0;t<Math.ceil(this.size*.5);t++){
                this.points.push(new Circle(this.center.x+(Math.cos(this.angle+(t*this.step))*(this.distances[t])), this.center.y+(Math.sin(this.angle+(t*this.step))*this.distances[t]), 2, this.color))
            }
            for(let t = Math.ceil(this.size*.5); t>= 0;t--){
                this.points.push(new Circle(this.center.x+(Math.cos(this.angle-(t*this.step))*(this.distances[t])), this.center.y+(Math.sin(this.angle-(t*this.step))*this.distances[t]), 2, this.color))
            }
            this.eye1 = new Circle(this.center.x+(Math.cos(this.angle-.7)*5),   this.center.y+(Math.sin(this.angle-.7)*5),  1.5, "#090909" )
            this.eye2 = new Circle(this.center.x+(Math.cos(this.angle+Math.PI*2+.7)*5),   this.center.y+(Math.sin(this.angle+Math.PI*2+.7)*5),  1.5, "#090909" )
        }
        draw(){
            this.center.draw()
            this.poly = new Polygon(this.center.x, this.center.y, 1, this.color, 10)
            this.poly.draw(this.points)
            // for(let t = 0;t<this.points.length;t++){
            //     // this.points[t].draw()
            // }
            this.eye1.draw()
            ...
            //     let link = new LineOP(this.points[t], this.points[t-1], this.color, this.points[t].radius*2)
            //     link.draw()
            // }
        }
    ...
    class Thorax{
        constructor(color){
            ...
            this.abdomendis = 18
            this.color = color
            this.center = new Circle(320,350, 1, this.color)
            this.points = []
            this.distances = []
            this.min = 15
            this.max = 1.5
        ...
        rebuild(){
            ...
            for(let t = -1;t<=1;t++){
                this.legs.push(new Leg(this.center))
                this.legs[t+1].angle += (t*Math.PI/8) + this.angle
            }
        }
        draw(){
            this.center.draw()
    ...
    class Leg{
        constructor(center){
            ...
            this.length = 19 //+ (Math.random()*2)
            this.point1 = new Circle(this.center.x+(Math.cos(this.angle)*this.length), this.center.y+(Math.sin(this.angle)*this.length), 2, this.center.color)
            this.link1 = new LineOP(this.center, this.point1, this.center.color, 2)
            this.point2 = new Circle(this.center.x+(Math.cos(this.angle+Math.PI)*this.length), this.center.y+(Math.sin(this.angle+Math.PI)*this.length), 2, this.center.color)
            this.link2 = new LineOP(this.center, this.point2, this.center.color, 2)
            this.step = 0
        }
        draw(){
                this.step+=.1
                this.point1 = new Circle(this.center.x+(Math.cos(this.angle)*this.length), this.center.y+(Math.sin(this.angle)*this.length), 2, this.center.color)
                this.link1 = new LineOP(this.center, this.point1, this.center.color, 3)
                this.point2 = new Circle(this.center.x+(Math.cos(this.angle+Math.PI)*this.length), this.center.y+(Math.sin(this.angle+Math.PI)*this.length), 2, this.center.color)
                this.link2 = new LineOP(this.center, this.point2, this.center.color, 3)
                this.point12 = new Circle(this.point1.x+(Math.cos(this.angle+.5)*this.length), this.point1.y+(Math.sin(this.angle+.5)*this.length), 2, this.center.color)
                this.link12 = new LineOP(this.point12, this.point1, this.center.color, 2)
                this.point22 = new Circle(this.point2.x+(Math.cos(this.angle+Math.PI-.5)*this.length), this.point2.y+(Math.sin(this.angle+Math.PI-.5)*this.length), 2, this.center.color)
                this.link22 = new LineOP(this.point22, this.point2, this.center.color, 2)

            // this.angle += Math.sin(this.step)*.01
            this.link1.draw()
            this.link2.draw()
            this.link12.draw()
            this.link22.draw()
    ...
    class Abdomen{
        constructor(color){
            this.color = color
            this.center = new Circle(280,350, 1, this.color)
            this.points = []
            this.distances = []
            this.min = 12
            this.max = 10
    ...
    class Beetle{
        constructor(){
            ...
            this.head = new Head(getRandomDarkColor())
            this.head.angle = this.angle
            this.color = getRandomDarkColor()
            this.thorax = new Thorax(this.color)
            this.thorax.angle = this.angle
            this.abdomen = new Abdomen(this.color)
            this.abdomen.angle = this.angle
            this.rebuild()
        }
        draw(){
            this.thorax.draw()
            ...
            if(keysPressed['w']){
                this.thorax.center.x -= (this.thorax.center.x-this.head.center.x)*.12
                this.thorax.center.y -= (this.thorax.center.y-this.head.center.y)*.12
                this.rebuild()
            }
            if(keysPressed['a']){
                this.angle += .09
        ...
        rebuild(){
            this.head.center.x = this.thorax.center.x+(Math.cos(this.angle)*this.thorax.headdis)
            this.head.center.y = this.thorax.center.y+(Math.sin(this.angle)*this.thorax.headdis)
            this.abdomen.center.x = this.thorax.center.x+(Math.cos(this.angle+Math.PI)*this.thorax.abdomendis)
            this.abdomen.center.y = this.thorax.center.y+(Math.sin(this.angle+Math.PI)*this.thorax.abdomendis)
    ...
    function main() {
        ...
        // // game code goes here
        for(let t = 0;t<beetles.length;t++){
            beetles[t].draw()
            // beetles[t] = new Beetle()
            // beetles[t]
        }
    }
})
